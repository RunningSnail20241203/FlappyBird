...
public class ViewModelLifetimeDrawer : PropertyDrawer
{
    ...
    private void CacheTypes()
    {
        ...
        typeDisplayNames = scriptableObjectTypes
    ...
    private void DrawTypeDropdown(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        if (!string.IsNullOrEmpty(currentTypeName))
        {
            ...
            if (currentType != null)
            {
                ...
                selectedIndex = Array.IndexOf(typeDisplayNames, currentDisplayName);
        ...
        selectedIndex = EditorGUI.Popup(position, label.text, selectedIndex, typeDisplayNames);
        if (EditorGUI.EndChangeCheck() && selectedIndex >= 0 && selectedIndex < scriptableObjectTypes.Length)
        {
            property.stringValue = scriptableObjectTypes[selectedIndex].AssemblyQualifiedName;
